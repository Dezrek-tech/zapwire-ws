<?php
    class User {
        function create($email, $password, $darkmode = 0, $activated = 0, $deactivated = 1) {
            global $app;
            ...
            $create_query = mysqli_query($app->_db_, "INSERT INTO `zw_users` (`id`, `email`, `password`, `avatar`, `deactivated`, `is_activated`, `have_logged_in`, `darkmode`) VALUES (NULL, '$email', '$password', '$user_avatar', '$deactivated', '$activated', 0, '$darkmode')");
        ...
        function get_by_year($year = '*') {
            ...
            $arr = array();
            while($row = mysqli_fetch_assoc($get_by_year_query)) {
                $arr[] = $row;
            }

            $get_by_year_result = $arr;
        ...
        function send_mail ($user, $subject, $content, $from) {
            ...
            $sendgrid_mail = new SendGrid_Mail();
            if($sendgrid_mail->send($app->project_info['noreply_email'], $user_email, $subject , $mail, 'text/html', $from)) {
                return 1;
            } else {
                return 0;
            }
        ...
        function mail( $subject, $content, $from = "Zappie from Zapwire :)", $user_id = '*') {
            ...
            if(@$user_id[0]) {
                ...
                foreach ($user_id as $id) {
                    ...
                    if(!$this->send_mail($user, $subject, $content, $from)) {
                        $sent--;
                    } else {
                        $sent++;
                    }
                ...
                foreach($this->fetch_all() as $user){
                    $sent = $this->send_mail($user, $subject, $content, $from);
                }
            } else {
